...
const Calculators = () => {
  ...
  const items = [
    { title: 'Աշխատավարձի հաշվիչ', to: '/calculators/salary', icon: Calculator, desc: 'Հաշվեք մաքուր աշխատավարձը (նետ) հարկերից հետո' },
    { title: 'Շրջհարկի հաշվիչ', to: '/calculators/vat', icon: Percent, desc: 'Հաշվեք ԱԱՀ-ը և գումարները` մաքուր/ընդհանուր' },
    { title: 'Շահութահարկի հաշվիչ', to: '/calculators/profit-tax', icon: Coins, desc: 'Հաշվեք շահութահարկը և մաքուր շահույթը' },
    { title: 'Նպաստի հաշվիչ', to: '/calculators/benefit', icon: HandCoins, desc: 'Սոցիալական նպաստների նախնական հաշվարկ' },
    { title: 'Նախագծերի հաշվիչ (Սմետա)', to: '/calculators/estimate', icon: ClipboardList, desc: 'Ծախսերի նախահաշիվ՝ սմետա' },
  ];
      ...
      <section className="py-12 sm:py-16 lg:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <header className="max-w-3xl mx-auto text-center mb-10 sm:mb-14">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold gradient-text">Ֆինանսական հաշվիչներ</h1>
            <p className="text-gray-300 mt-4">Ընտրեք հաշվիչը՝ բացելու համար իր առանձնահատկություններով</p>
          </header>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {items.map(({ title, to, icon: Icon, desc }) => (
